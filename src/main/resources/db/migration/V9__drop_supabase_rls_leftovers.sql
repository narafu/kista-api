...
-- 위 함수(또는 Supabase 대시보드)로 인해 정책(Policy) 없이 RLS만 켜져 있던 테이블 정리.
...
-- flyway_schema_history는 제외: Flyway가 마이그레이션 실행 중 이 테이블에 advisory lock 연결로 AccessShareLock을
-- 계속 쥐고 있어, 같은 마이그레이션 안에서 ALTER TABLE(AccessExclusiveLock 요구)을 실행하면 영구 대기(hang)로 죽는다.
