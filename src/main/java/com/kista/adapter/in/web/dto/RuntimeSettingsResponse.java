...
public record RuntimeSettingsResponse(
    ...
    public record AuthResponse(boolean approvalRequired) { // 가입 승인 공개 설정
    }

    public record BrokerResponse(boolean enabled) { // 증권사 신규 등록 공개 설정
    }

    @JsonInclude(JsonInclude.Include.NON_NULL)
    public record StrategyResponse(boolean enabled, FieldResponses fields) { // 전략 생성 공개 설정
