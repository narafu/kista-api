...
interface FinanceTransactionJpaRepository extends JpaRepository<FinanceTransactionEntity, UUID> {
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM finance_transactions WHERE group_id = :groupId " +
    ...
    List<FinanceTransactionEntity> findByGroupId(@Param("groupId") UUID groupId, @Param("from") LocalDate from,
            @Param("to") LocalDate to, @Param("categoryId") UUID categoryId, @Param("createdBy") UUID createdBy);
    ...
    @Modifying
    @Query("UPDATE FinanceTransactionEntity t SET t.deletedAt = :now WHERE t.createdBy = :createdBy AND t.deletedAt IS NULL")
    ...
    @Query("UPDATE FinanceTransactionEntity t SET t.groupId = :toGroupId " +
            "WHERE t.groupId = :fromGroupId AND t.createdBy = :createdBy")
