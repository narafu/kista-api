...
class PrivacyTradePersistenceAdapter implements PrivacyTradePort {
    ...
    private static final Comparator<FidaPlannedOrder> ORDER_SORT =
            orderSort(FidaPlannedOrder::direction, FidaPlannedOrder::price);
    ...
    public PrivacyTradeSaveResult saveBaseWithOrders(FidaOrderCommand command) {
        ...
        List<FidaPlannedOrder> sorted = command.orders().stream().sorted(ORDER_SORT).toList();
        for (FidaPlannedOrder order : sorted) {
    ...
    private boolean isIdentical(PrivacyTradeBaseEntity base, FidaOrderCommand command) {
        ...
        List<PrivacyTradeBaseOrderEntity> existingOrders = base.getOrders().stream()
                ...
                .toList();
        List<FidaPlannedOrder> incomingOrders = command.orders().stream().sorted(ORDER_SORT).toList();
        ...
        for (int i = 0; i < existingOrders.size(); i++) {
            ...
            FidaPlannedOrder o = incomingOrders.get(i);
