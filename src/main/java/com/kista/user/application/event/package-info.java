...
// UserReappliedEvent. notify 등 타 모듈이 @TransactionalEventListener로 구독한다
// (CLOSED↔CLOSED 모듈 간 이벤트 교차). "event" 이름으로 공개된다.
// UserDeletedEvent는 trading-core 모듈 분리 시 com.kista.sharedkernel로 승격됐다 — trading-core(구독처)가
// root(발행처)에 컴파일 역의존하지 않도록 root가 의존 가능한 sharedkernel로 소유를 옮긴 것(컨트롤러 룰링).
