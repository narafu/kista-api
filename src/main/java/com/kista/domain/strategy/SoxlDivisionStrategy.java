...
public class SoxlDivisionStrategy implements TradingStrategy {
    ...
    public TradingVariables calculate(AccountBalance balance, BigDecimal currentPrice) {
        ...
        BigDecimal S = calcPriceOffsetRate(T);
        BigDecimal G = calcReferencePrice(A, S);
        BigDecimal P = calcTargetPrice(A);
        ...
        return TradingVariables.builder()
                ...
                .currentRound(T).unitAmount(K).targetProfitRate(TARGET_PROFIT_RATE)
    ...
    // 기준률: 0.20 × (1 − 2T/20) (scale=4, HALF_UP)
    private BigDecimal calcPriceOffsetRate(double currentRound) {
        return TARGET_PROFIT_RATE.multiply(
    ...
    // 목표가: A × 1.20 (scale=2, HALF_UP)
    private BigDecimal calcTargetPrice(BigDecimal averagePrice) {
        return averagePrice.multiply(BigDecimal.ONE.add(TARGET_PROFIT_RATE))
                .setScale(2, RoundingMode.HALF_UP);
    }

    @Override
    public List<Order> buildOrders(TradingVariables vars, LocalDate tradeDate, String symbol) {
