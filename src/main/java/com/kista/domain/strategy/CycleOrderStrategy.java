...
// 전략 패턴 진입점 — TradingService/TradingPreviewService/CycleRotationService 의 switch(cycle.type()) 분기를 다형성으로 대체
// 각 구현체는 cycleType()으로 자기 타입을 선언하며, 서비스는 Map<TradingCycle.Type, CycleOrderStrategy>로 주입받아 사용
public interface CycleOrderStrategy {

    // 이 전략이 담당하는 사이클 타입
    TradingCycle.Type cycleType();
