...
class TradingReporterTest {
    ...
    void 체결_내역이_없는_PLACED_주문은_CANCELLED_처리된다() {
        ...
        when(executionPort.getExecutions(TODAY, TODAY, Ticker.SOXL, ACCOUNT_REF))
    ...
    void 체결이_전혀_없어도_미체결_PLACED_주문은_CANCELLED_처리된다() {
        ...
        when(executionPort.getExecutions(TODAY, TODAY, Ticker.SOXL, ACCOUNT_REF)).thenReturn(List.of());
    ...
    void 마감_리포트는_체결_조회_전에_Toss_잔여_PLACED_주문을_취소한다() {
        ...
        when(executionPort.getExecutions(TODAY, TODAY, Ticker.SOXL, TOSS_ACCOUNT_REF)).thenReturn(List.of());
        ...
        inOrder.verify(executionPort).getExecutions(TODAY, TODAY, Ticker.SOXL, TOSS_ACCOUNT_REF);
    }
    ...
    void Toss_취소_실패는_격리되고_관리자_알림으로_표면화되며_체결조회는_계속된다() {
        ...
        when(executionPort.getExecutions(TODAY, TODAY, Ticker.SOXL, TOSS_ACCOUNT_REF))
