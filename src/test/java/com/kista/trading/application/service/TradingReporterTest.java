...
class TradingReporterTest {
    ...
    void 마감_리포트는_체결_조회_전에_Toss_잔여_PLACED_주문을_취소한다() {
        ...
        inOrder.verify(brokerOrderPort).cancel(new CancelInstruction(order.ticker(), order.externalOrderId()), TOSS_ACCOUNT);
    ...
    void Toss_취소_실패는_격리되고_관리자_알림으로_표면화되며_체결조회는_계속된다() {
        UUID orderId = UUID.randomUUID();
        Order order = tossPlacedOrder(orderId, "E1", 5);
        doThrow(new RuntimeException("이미 체결된 주문")).when(brokerOrderPort).cancel(new CancelInstruction(order.ticker(), order.externalOrderId()), TOSS_ACCOUNT);
    ...
    void Toss_취소가_이미체결_409로_거부되면_관리자_알림없이_체결로_기록된다() {
        ...
        doThrow(new TossApiException(
                ...
                null, TossApiException.Conflict.ALREADY_FILLED)).when(brokerOrderPort).cancel(new CancelInstruction(order.ticker(), order.externalOrderId()), TOSS_ACCOUNT);
        when(executionPort.getExecutions(TODAY, TODAY, Ticker.SOXL, TOSS_ACCOUNT))
                .thenReturn(List.of(buyExecution("E1", 5, "20.00")));
