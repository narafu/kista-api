...
class UserJpaRepositoryCountTest extends DataJpaTestBase {
    ...
    private void insertUser(User.UserStatus status, boolean deleted) {
    ...
    void countGroupByStatus_상태별_사용자수를_집계하고_soft_delete는_제외한다() {
        insertUser(User.UserStatus.PENDING, false);
        insertUser(User.UserStatus.ACTIVE, false);
        insertUser(User.UserStatus.ACTIVE, false);
        insertUser(User.UserStatus.REJECTED, false);
        insertUser(User.UserStatus.ACTIVE, true); // soft-deleted — 집계 제외 대상

        List<UserJpaRepository.StatusCountProjection> result = userJpaRepository.countGroupByStatus();

        long pending = result.stream().filter(p -> p.getStatus() == User.UserStatus.PENDING).mapToLong(UserJpaRepository.StatusCountProjection::getCount).sum();
        long active = result.stream().filter(p -> p.getStatus() == User.UserStatus.ACTIVE).mapToLong(UserJpaRepository.StatusCountProjection::getCount).sum();
        long rejected = result.stream().filter(p -> p.getStatus() == User.UserStatus.REJECTED).mapToLong(UserJpaRepository.StatusCountProjection::getCount).sum();
