...
- `DashboardController`: DB 기반 전용 DTO 반환 — `GET /api/accounts/{accountId}/cycle-history` → `CycleHistoryPageResponse` (커서 페이지네이션). 이 DTO는 stats(`DashboardController`, 계좌 전체 이력)와 trading(`TradingCycleController.getStrategyHistory`, 단일 전략 이력 — 2026-09-10 web에서 이관됨)에 byte-identical 사본이 각각 존재, 2026-09-10 재검증: 둘 다 여전히 살아있는 별개 엔드포인트
