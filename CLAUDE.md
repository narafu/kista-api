...
- **보일러플레이트 즉시 수정**: 반복 생성자 호출(전 필드 복붙), 중복 null guard, `with*()` 메서드로 대체 가능한 코드를 발견하면 제안 없이 즉시 수정
...
- **kista-ui 연계 작업 감지 시**: API 응답 형식 변경, 인증/토큰 흐름 등 kista-ui와 결합된 작업이면 즉시 `../kista-ui/CLAUDE.md`를 Read로 확인할 것 — 세션 시작 디렉토리가 아닌 저장소의 CLAUDE.md는 자동 로드되지 않음
