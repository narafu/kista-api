...
- **서브에이전트 실행 기본 원칙**: 독립적으로 병렬 처리 가능한 작업(파일이 겹치지 않는 작업)은 서브에이전트로 병렬 진행을 기본값으로 하며, 플랜 수립 시 오케스트레이션 모델과 각 태스크별 서브에이전트 모델(구현은 효율적인(저렴한) 모델, 완료 후 검수하는 검토자는 상위 모델 기본값)을 명시하고 검수까지 거칠 것
...
- **커밋 전 검토자 검수 의무화**: 코드를 변경하는 모든 작업은 커밋 직전에 반드시 별도 검토자(리뷰어 서브에이전트 또는 code-review 계열 skill/workflow)의 검수를 거칠 것 — 직접 구현했든 서브에이전트에게 위임했든 관계없이 적용. 검토자가 발견한 실제 결함은 커밋 전에 수정·재검증한다. 문서 전용 변경은 예외
- **작업 완료 후 자동 커밋**: 요청된 작업 완료 시(코드 변경이면 위 검토자 검수까지 끝난 뒤) 스스로 커밋 생성
...
- **DB 작업**: 자체호스팅 postgres(`kista-postgres` 컨테이너) — SSH 접속 후 `docker exec kista-postgres psql -U kista -d kistadb` (Supabase는 2026-08-14 프로젝트 삭제 완료 — 상세 명령어 → `docs/agents/docker-infra.md`)
