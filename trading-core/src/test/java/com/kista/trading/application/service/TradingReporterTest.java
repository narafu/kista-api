...
class TradingReporterTest {
    ...
    private static Order placedOrder(UUID id, String externalOrderId, int quantity) {
        return new Order(id, ACCOUNT.id(), CYCLE.id(), TODAY, StrategyTicker.SOXL,
                ...
                quantity, new BigDecimal("20.00"), OrderStatus.PLACED,
    ...
    void 체결_내역이_없는_PLACED_주문은_CANCELLED_처리된다() {
        ...
        verify(orderPort).markFilled(filledId, 3, new BigDecimal("20.00"), OrderStatus.FILLED);
    }
    ...
    void 부분_체결은_PARTIALLY_FILLED와_가중평균가를_기록한다() {
        ...
        verify(orderPort).markFilled(orderId, 5, new BigDecimal("20.40"), OrderStatus.PARTIALLY_FILLED);
    }
    ...
    void TRADING_ALERT_비활성이면_리포트를_발송하지_않는다() {
        TradingUserProfile mutedProfile = new TradingUserProfile(USER.userId(),
                Map.of(NotificationType.TRADING_ALERT, false), true, null, null);
        BatchContext mutedCtx = new BatchContext(STRATEGY, CYCLE, ACCOUNT, mutedProfile);
        when(executionPort.getExecutions(TODAY, TODAY, StrategyTicker.SOXL, ACCOUNT_REF)).thenReturn(List.of());

        reporter.recordAndNotify(TODAY, mutedCtx, BALANCE, CLOSE, List.of(), null);
    ...
    void Toss_취소_실패는_격리되고_관리자_알림으로_표면화되며_체결조회는_계속된다() {
        ...
        verify(orderPort).markFilled(orderId, 5, new BigDecimal("20.00"), OrderStatus.FILLED);
    }
