...
@Slf4j
public class TradingBalanceLoader {

    private final CyclePositionPort cyclePositionPort;

    // 잔고 로드 결과 — 정상이면 balance non-null, skip이면 skipReason non-null
    public record BalanceLoad(AccountBalance balance, SkipReason skipReason) {
    ...
    public BalanceLoad tryLoadBalance(Strategy strategy) {
    ...
    public BalanceLoad loadBalanceOrThrow(Strategy strategy) {
