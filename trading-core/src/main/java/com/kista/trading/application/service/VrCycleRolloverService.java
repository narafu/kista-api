...
class VrCycleRolloverService {
    ...
    void rollIfDue(BatchContext ctx, AccountBalance postBalance, BigDecimal closingPrice, LocalDate today) {
        ...
        if (adjustedPool.compareTo(BigDecimal.ZERO) < 0) {
            ...
            eventPublisher.publishEvent(new TradingErrorEvent(ctx.userProfile().userId(),
        ...
        eventPublisher.publishEvent(new NewCycleStartedEvent(ctx.userProfile().userId(), ctx.account().id(), ctx.account().nickname(),
                strategy.type(), strategy.ticker(), adjustedPool));
    }
