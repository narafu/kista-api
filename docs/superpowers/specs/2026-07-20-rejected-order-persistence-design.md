...
- `NextOrdersPreview.todayPlannedOrders`(API 응답 필드명 `todayOrders`)와 별도 `rejectedOrders` 필드 분리, kista-ui `mode`/카운트/취소버튼 가드 — 불필요 (주문 목록 자체를 안 건드림)
...
- `StrategyDetail.tsx`에서 `<BuyCompetitionNotice .../>` 사용처 **두 곳을 모두 제거**한다 — 새 거절 배너가 사실 그대로의 단일 정보원이 되므로, 근사치 재시뮬레이션 배지를 "다음 주문" 카드에 남겨두면 서로 다른 근거(라이브 재계산 vs 실측 이력)로 같은 말을 반복해 오히려 복잡도를 늘린다.
  - 헤더 인라인(`:229-231`, `variant="inline"`, `hidden lg:flex` — 데스크톱 전용)
  - `CardContent` 내부 preview 분기의 row-variant(`:308-310`, `variant="row"`, `lg:hidden` — 모바일 전용)
  - **주의**: 두 인스턴스는 데스크톱/모바일로 반응형 분할된 동일 개념의 배지다 — 헤더 쪽만 지우면 데스크톱 preview 모드에서는 배지가 사라지고 모바일 preview 모드에서는 그대로 남는 비대칭이 생긴다. 반드시 둘 다 같이 제거한다.
  - 제거 후 `BuyCompetitionNotice` 컴포넌트는 어디서도 쓰이지 않으므로(`grep -rl BuyCompetitionNotice` 결과가 자기 자신의 테스트 파일뿐이 됨) `widgets/strategy-detail/BuyCompetitionNotice.tsx`와 `BuyCompetitionNotice.test.tsx`를 함께 삭제한다.
  - `competition`/`hasDeficit`/`deficitUsd` 계산 로직 자체와 `preview.competition` 필드는 그대로 둔다 — `StrategyDetail.tsx` 자신도 "바로 주문" 버튼 옆 소형 배지(`:235-237`, `{isHoliday ? '휴장일' : '예수금 부족'}`)와 클릭 가드(`:245-247`)에서 `hasDeficit`을 계속 쓰고, **`StrategyCard.tsx`(전략 목록 카드)는 같은 `competition`을 목록 카드 테두리 색상(예수금 부족 시 빨간 테두리)에 별도로 쓰고 있어** 이번 변경 범위(다음 주문 카드)를 벗어나 목록 페이지까지 건드리지 않는다. 백엔드 `TradingBuyCompetitionSimulator`/`BuyCompetitionPreview`도 변경 없음.
