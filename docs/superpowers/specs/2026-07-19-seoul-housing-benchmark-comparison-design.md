...
- 해당 일에 활성인 모든 전략의 선택된 사이클에 완전한 평가값이 모이기 전에는 포트폴리오 일별 평가값을 만들지 않는다.
- 같은 날 사이클이 교체되면 신규 사이클만 교체 후 평가값으로 사용하며 이전·신규 사이클을 중복 합산하지 않는다.
- 프로젝트 롤오버는 이전 사이클 `endDate`와 다음 사이클 `startDate`가 같은 날 생성된다. 두 날짜가 같을 때만 연속된 내부 교체로 취급한다.
- 두 날짜가 다르면 다음 날 시작을 포함해 비연속으로 본다. 이전 사이클은 `endDate`에 최종 회수하고, 다음 사이클은 `startDate`에 새 외부 진입으로 처리한다.
- 다음 사이클이 없는 전략은 마지막 사이클의 `endDate`에 일말 회수된 것으로 보고 해당 일의 교체 후 포트폴리오 평가값에서 제외한다.
...
- 각 전략의 최초 외부 유입액만 첫 사이클의 첫 완전평가액으로 기록한다. 초기 스냅샷에 보유주식이 있으면 `usd_deposit + holdings * closing_price` 전체를 초기 외부자금으로 본다.
- 같은 전략의 내부 사이클 교체는 다음 사이클의 첫 완전평가액에서 교체 시점 이전 사이클의 마지막 완전평가액을 뺀 차이만 일말 외부 흐름으로 본다. 레거시 `endAmount`는 현금만 저장했을 수 있으므로 교체 흐름 계산에 사용하지 않는다.
- 다음 사이클의 첫 완전평가액이 이전 사이클의 마지막 완전평가액과 같으면 보유주식까지 전액 이월된 것이므로 흐름은 0이다. 다음 값이 더 작으면 차액은 회수, 더 크면 차액은 추가 투입이다.
- 예를 들어 전일 평가액 100, 당일 이전 사이클 마지막 완전평가액 110, 다음 사이클 첫 완전평가액 150이면 일말 외부 유입은 40이고 일별 수익률은 `(150 - 40) / 100 - 1 = 10%`다.
- 다음 사이클이 없는 전략은 마지막 사이클의 `endDate`에 마지막 완전평가액 전체를 음수 흐름으로 기록하고, `V(d)`에서는 해당 전략을 제외한다.
- 최종 회수로 활성 전략이 하나도 남지 않더라도 직전 유효 평가액이 있고 해당 일의 순흐름이 음수이면 그날만 `V(d)=0`인 교체 후 포트폴리오를 허용한다. 이외의 빈 포트폴리오나 불완전 포트폴리오는 일별 평가값으로 만들지 않는다.
...
- `MonthlyReturnCalculatorTest`
  ...
  - VR 롤오버의 현금과 보유주식 전체가 같으면 내부 흐름 0
  - 최종 전략 종료일에 마지막 전체 평가액 회수와 생존 전략 수익 보존
  - 단독 전략 최종 종료일의 0인 교체 후 평가액으로 마지막 수익 보존
  - 같은 전략의 비연속 사이클은 종료 회수와 신규 진입으로 분리
