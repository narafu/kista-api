...
토스증권 공식 문서는 `dateTime`으로 특정 시점 환율을 조회할 수 있음을 명시하지만 과거 조회 가능 기간과 호출 제한을 구체적으로 공개하지 않는다. 구현 시작 시 오래된 월 3개 이상을 대상으로 스파이크 테스트해 실제 조회 범위, 비영업일 동작, 응답의 `midRate`를 확인한다. 전체 기간 조회가 제한되면 구현을 중단하고 대체 환율 소스를 설계한다. 검증되지 않은 환율로 차트를 제공하지 않는다.
...
- 최초 배포 시 KB 서울 데이터와 투자 기록이 겹치는 월을 대상으로 별도 backfill use case를 실행한다.
