...
1. **잔고 급변 안전장치(balance-spike safety guard)** — ❌ **진행 안 함(사용자 결정, 2026-07-17)**. 브레인스토밍 재개 없이 백로그에서 제외 — 사용자가 재요청하지 않는 한 재론 금지. (참고용 기록: 삽입 지점은 `TradingService.notifyIfInsufficientLiveBalance()` 근처로 조사됐었고, 부분 결정은 "알림만 발송, 주문은 그대로 진행" + "비율 기준(예수금 또는 보유수량이 직전 대비 50% 이상 변동)"이었음.)
2. **브로커 서킷브레이커** — ❌ **진행 안 함(사용자 결정, 2026-07-17)**. 브레인스토밍 재개 없이 백로그에서 제외 — 사용자가 재요청하지 않는 한 재론 금지. (참고용 기록: 스코프는 "브로커 단위"가 `BrokerAdapterRegistry`의 `Map<Broker, Port>` 구조와 잘 맞는다고 조사됐었고, open 조건·open 이후 동작 질문에서 중단된 상태였음.)
...
**권장 우선순위**: 4번(스케쥴러 인터럽트 사용자 알림)이 가장 작고 명확한 스코프이며 실제 사용자 피해(누락된 주문을 모르는 상태)와 직결되므로 트랙 C에서는 1순위로 제안했고 완료됨. 1번·2번은 2026-07-17 사용자 결정으로 진행 안 함 확정. 5번은 별도 이니셔티브라 순서 밖.
...
4. **진행 안 함 확정**: 트랙 C(1+2, 잔고 급변 가드+서킷브레이커), 트랙 C(5, SaaS 상용화 갭) — 사용자 결정(2026-07-17)으로 백로그에서 제외, 재요청 없이는 재론 금지.
