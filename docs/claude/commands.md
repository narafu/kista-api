...
# 테스트
bash gradlew test                             # 전체 테스트 (병렬 실행)
bash gradlew compileJava                      # 컴파일만 빠르게 확인 (테스트 없이)
bash gradlew test --tests "com.kista.architecture.*"    # ArchUnit 규칙 테스트만
bash gradlew test --tests "com.kista.domain.*"          # 도메인 단위 테스트만
bash gradlew test --tests "com.kista.adapter.out.kis.*" # KIS Adapter 단위 테스트만
bash gradlew test --rerun-tasks               # 캐시 무시하고 강제 재실행 (UP-TO-DATE 우회)
