## 스케줄러 실행 흐름
- 스케줄러 기동: 04:00 KST → `TradingCycleRepository.findAllActive()`로 ACTIVE 사이클 목록 조회
- 사이클별 순차 실행: `TradingService.execute(TradingCycle, Account, User)` — 한 사이클 실패 시 다음 계속 (격리)
- 각 사이클: 잔고·현재가 조회 → 전략 계산 → `orders` PLANNED 저장 → `DstInfo.waitUntilOrderTime()` 대기 (DST=30분, 비DST=90분) → `orders` 조회 → KIS 접수 (PLACED 기록)
