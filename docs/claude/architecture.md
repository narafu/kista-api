...
| `PortfolioSnapshot` record 필드 추가/제거 | `PortfolioSnapshotEntity` + `PortfolioSnapshotPersistenceAdapter`(toEntity/toDomain) + `PortfolioSnapshotResponse.from()` + `TradingService.toSnapshot()` + `TelegramBotService.buildStatusMessage()` + 테스트 4개(`PortfolioSnapshotPersistenceAdapterTest`, `DashboardControllerTest`, `TelegramBotServiceTest`, `PortfolioServiceTest`) + Flyway 마이그레이션 |
