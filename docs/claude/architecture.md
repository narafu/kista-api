...
domain/          ← 순수 Java record/class. Spring·JPA 어노테이션 금지
  model/         ← 불변 값 객체 (record 사용), 어그리게이트별 서브패키지
    ...
    toss/        ← TossApiException (→ GlobalExceptionHandler 503), TossAccountInfo, TossCandle, TossExchangeRate, TossMarketSession, TossSellableQuantity, TossStockInfo
...
adapter/out/
  ...
  persistence/   ← JPA 인프라 (BaseAuditEntity, BaseCreatedAtEntity, JpaAuditingConfig) + 어그리게이트별 서브패키지
    ...
    settings/    ← UserSettingsJpaEntity + UserSettingsJpaRepository + UserSettingsPersistenceAdapter
