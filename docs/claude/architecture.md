...
domain/          ← 순수 Java record/class. Spring·JPA 어노테이션 금지
  model/         ← 불변 값 객체 (record 사용), 어그리게이트별 서브패키지
    ...
    account/     ← Account(+nested exceptions), Broker, StrategyType, StrategyStatus
    strategy/    ← InfinitePosition, AccountBalance, TradingSnapshot, TradingReport, Ticker, DstInfo
    order/       ← Order, PlannedOrder, TradeHistory, TradeEvent, PortfolioSnapshot, ReservationOrderCommand
...
- `privacy_trades_detail`: 마스터 1행에 대한 계획 주문 세트 (direction/orderType/qty/price) — 향후 필드 추가 예정
- 스케줄러 흐름: `StrategyType.PRIVACY` → `privacy_trades_master/detail` 조회 → `planned_orders` 복사 (미구현, INFINITE와 동일 출구)
