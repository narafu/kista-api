...
| `TradingCycleHistory` record 필드 추가/제거 | `TradingCycleHistoryEntity` + `TradingCycleHistoryPersistenceAdapter`(toDomain/toEntity) + `TradingCycleService.register()` + `TradingService.saveCycleHistory()` + `TradingService.rotateCycleIfConsecutive()` + `TradingServiceTest` |
...
### PRIVACY buildOrders() 알고리즘 (변경 금지)
- 진입부 동적 배수 산출: `multiple = floor(initialUsdDeposit / base.currentCycleStart(), 2)` — null/0 이하 시 `IllegalStateException`
- BUY 가감: `target = privacyTradeBase.holdings() × multiple (DOWN)`, `diff = target - balance.holdings()`
  - diff > 0 (보유 부족): 가장 싼 BUY 1건에 diff 전량 추가
  - diff < 0 (보유 초과): 가장 비싼 BUY부터 순차 차감, quantity=0이면 결과에서 제외
- SELL null quantity = "잔량 전부": `remaining = balance.holdings() - Σ(명시 SELL × multiple)`
  - remaining > 0 이면 null SELL quantity = remaining (`applyMultiple` 재적용 금지)
  - remaining ≤ 0 이면 null SELL 제외 + log.warn
  - null SELL 2개 이상이면 `IllegalStateException`
- BUY null quantity는 기준표 오류로 간주 → log.warn + skip (SELL null과 다름)
