...
domain/          ← 순수 Java record/class. Spring·JPA 어노테이션 금지
  model/         ← 불변 값 객체 (record 사용), 어그리게이트별 서브패키지
    ...
    tradingcycle/ ← TradingCycle(+nested: Type, Status, Ticker, CycleSeedType), TradingCyclePosition, AccountCycleHistoryEntry
...
application/
  service/       ← UseCase 구현체 (package-private @Service), Port를 통해서만 외부 호출
                    - TradingExecutionFacade — TradingExecutionUseCase 구현 (Facade)
                      preview/executeManually/cancelOrder/cancelByCycle/execute/executeBatch 단일 진입점
                    - TradingService — 배치·단건 실행 코어 (package-private)
                    - ManualTradingService — 수동 실행 (package-private)
                    - TradingPreviewService — 미리보기 전용 (package-private, @Transactional readOnly)
                    - OrderCancelService — 주문 취소 (package-private)
                    - CycleRotationService — 사이클 종료 후 cycleSeedType 기반 재등록 (package-private)
                    - TradingBalanceLoader — 이력에서 AccountBalance 복원 helper (package-private)
                    - TradingOrderPlanner — 주문 계획 helper (package-private)
                    - TradingOrderExecutor — KIS 주문 접수 helper (package-private)
                    - TradingPriceFetcher — 현재가 조회·캐시 helper (package-private)
                    - TradingReporter — 매매 결과 알림 helper (package-private)
                    - BuyOrderPriceCapper — 매수 주문가 상한 조정 helper (package-private)
                    - CycleOrderComputer — 사이클별 주문 계산 helper (package-private)
                    - UserCascadeDeleter — 사용자 소프트 삭제 cascade helper

adapter/in/
  schedule/      ← TradingScheduler (월~금 04:00 KST, 멀티계좌) + MarketCalendarRefreshScheduler (1월 1일 3년치 / 매월 1일 최신화)
  web/           ← REST Controller + DTO
                    AuthController (카카오/JWT/승인/탈퇴/SSE), AccountController (계좌CRUD+연결테스트), TradingCycleController (사이클CRUD+pause/resume+수동실행), DashboardController (DB기반 포트폴리오/거래내역), KisStatisticsController (KIS live 잔고/수익/가격), MetaController (enum SSOT /api/meta/**, Cache 1h), OrderController+OrderCancelController, MarketHolidayController (휴장일/세션 DIRECT|BLOCKED), FidaOrderController (/api/internal/**, X-Internal-Token), SettingsController (텔레그램+알림채널), FcmController, TradeStreamController (SSE), PrivacyTradeController, Admin*Controller×5 (/api/admin/**), DevAuthController (local전용)
...
adapter/out/
  ...
  persistence/   ← JPA 인프라 (BaseAuditEntity, BaseCreatedAtEntity, JpaAuditingConfig) + 어그리게이트별 서브패키지
    ...
    tradingcycle/ ← TradingCycleEntity + TradingCycleJpaRepository + TradingCyclePersistenceAdapter
                     TradingCyclePositionEntity + TradingCyclePositionJpaRepository + TradingCyclePositionPersistenceAdapter
...
- **dev-token 소유권 제한**: `POST /api/auth/dev-token`은 고정 UUID(`...0001`) 소유 계좌만 접근 가능 — 403 시 DB에서 `trading_cycle_position` 직접 조회로 대체
