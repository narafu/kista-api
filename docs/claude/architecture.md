...
domain/          ← 순수 Java record/class. Spring·JPA 어노테이션 금지
  model/         ← 불변 값 객체 (record 사용), 어그리게이트별 서브패키지
    ...
    strategy/    ← Strategy(+nested: StrategyType, StrategyStatus, Ticker), InfinitePosition, AccountBalance, TradingSnapshot, TradingReport, DstInfo
...
adapter/in/
  ...
  web/           ← REST Controller + DTO
                    ...
                    - StrategyController: 전략 CRUD + pause/resume (GET/POST /api/accounts/{id}/strategies, PUT/DELETE/PATCH /api/strategies/{id})
...
adapter/out/
  ...
  persistence/   ← JPA 인프라 (BaseAuditEntity, JpaAuditingConfig) + 어그리게이트별 서브패키지
    ...
    strategy/    ← StrategyEntity + StrategyJpaRepository + StrategyPersistenceAdapter
...
| strategies 테이블 변경 | `StrategyEntity`(persistence/strategy/) + `StrategyJpaRepository` + `StrategyPersistenceAdapter` + `StrategyPersistenceAdapterTest` |
| `Strategy` record 필드 추가/제거 | `StrategyEntity` + `StrategyPersistenceAdapter`(toEntity/toDomain) + `StrategyService` + `StrategyRequest`/`StrategyResponse` + `StrategyControllerTest` + 테스트에서 `new Strategy(...)` 직접 생성 호출처 |
...
- `strategy.verifyOwnedBy(account)` — `strategy.accountId().equals(account.id())` 검증, 마찬가지로 `SecurityException`
- 전략 소유권 확인 순서: `strategyRepository.findByIdOrThrow(id)` → `accountRepository.findByIdOrThrow(strategy.accountId())` → `account.verifyOwnedBy(requesterId)`
- `accountRepository.findByIdOrThrow(id)` / `strategyRepository.findByIdOrThrow(id)` — 없으면 `NoSuchElementException` (컨트롤러에서 404 매핑)
...
- `InfinitePosition(AccountBalance, Strategy.Ticker, BigDecimal price, BigDecimal multiple)` — V35에서 `multiple` 추가. `unitAmount K = B ÷ 20 × multiple`
...
- `ExecuteTradingUseCase.execute(Strategy strategy, Account account, User user)` — V35에서 Strategy를 명시적으로 전달, 전략 단위 루프
- `TradingScheduler`: `strategyRepository.findAllActive()` → 전략 단위 loop (격리 try-catch)
