...
- 개장 경로: 동일한 leg-aware 후보·예산 배정을 수행하되 누락된 `AT_OPEN` 주문도 저장한다. `DstInfo.waitUntilMarketOpen()` 대기 후 접수 대상 ticker 현재가를 재조회(`reloadPlacementPrices`, 마감 경로와 동일 함수)하고, `TradingOrderExecutor.placeAtOpenOrders()`가 이 가격으로 `BuyOrderPriceCapper` BUY cap 보정을 적용한 뒤 `AT_OPEN` PLANNED 주문만 선접수한다. 마감 경로에서는 선접수된 주문도 포함해 중복 없이 후속 접수·리포트한다.
