...
- **전략 생성 화면과 실제 매매의 기준가 통일**: `AccountStatisticsService.getPrices()`(stats, `GET /prices` 티커 목록)와 `StrategyService.strategySeedPreview()`(trading, 최소 시드 미리보기) 모두 `BrokerPricePort.getPrevClose(s)`를 사용 — `TradingPreviewService`·실제 매매 실행(`InfinitePosition.averagePrice()`, holdings==0→prevClose)과 동일한 소스로 맞춰, "전략 생성 시 본 기준가"와 "실제 첫 주문가"가 항상 일치하도록 함. 순수 현재가(`getPrice`/`getPrices`)는 이 두 화면 어디에도 쓰지 않음
