...
- user(+auth) 애그리게이트(가입·승인·프로필·설정 + JWT/RefreshToken/블랙리스트/카카오 OAuth)는 `com.kista.user`로 이미 옮겨졌다 — 신규 관련 코드도 레거시 최상위가 아닌 `com.kista.user` 안에 추가. `domain/model`+`domain/auth`가 "domain"으로, `application/usecase`가 "usecase"로, `application/port/output`이 "port"로, `application/event`가 "event"로 NamedInterface 공개 — `application/service`·`adapter/*`·`config`는 비공개(internal). `User.UserRole`/`UserStatus`/`NotificationChannel` nested enum + `NotificationType`은 `com.kista.sharedkernel`(`@ApplicationModule(Type.OPEN)`) 독립 타입으로 이관 완료(아래 "User nested enum 패턴" 참고). admin에서 이관받은 `AdminUserView`/`AdminUserViewPort`, notify에서 이관받은 `TelegramBotInfoPort`는 물리 이전 후 `verify()` 실측으로 뒤늦게 발견된 admin/notify 순환 해소 산물(스펙 문서 "착수 순서 v2" 3번 항목 각주 참고)
