...
- account 애그리게이트(계좌 자격증명·브로커 연결)는 `com.kista.account`로 이미 옮겨졌다 — 신규 관련 코드도 레거시 최상위가 아닌 `com.kista.account` 안에 추가. `domain/model`이 "domain"으로, `application/usecase`가 "usecase"로, `application/port/output`이 "port"로, `application/event`가 "event"로 NamedInterface 공개 — `application/service`·`adapter/*`는 비공개(internal). `AccountStatisticsService`/`TossStatisticsService`/`BrokerStatisticsRouter`는 실질 stats 관심사라 account로 옮기지 않고 레거시 `com.kista.application.service.account` 패키지에 잔류(향후 stats 재배치 또는 strategy-config 이전 때 정리 대상). `AccountDeletedEvent`는 account가 발행하고 레거시 strategy 패키지의 `AccountCascadeListener`가 구독(strategy-config 자체가 아직 모듈이 아니라 리스너가 레거시 위치). `BrokerEnabledPort`는 admin↔account 포트 역전 산물(user `ApprovalPolicyPort`와 동일 패턴). broker의 11개 포트가 `Account` 대신 own-type `BrokerAccountRef`를 쓰는 것은 broker↔account 순환 해소 산물 — 물리 이전 실행 1차 시도의 `verify()`에서 실측 발견됐고(계획 작성 시점 사전조사가 놓친 순환), 아래 "모듈 경계 포트 시그니처" own-type 7번째 인스턴스로 해소했다. strategy-config는 이 이전 스코프에서 제외됐었다 — trading과의 결합(`CycleSnapshotCreator.reconfigureVrCycle`)이 실측으로 드러나 별도 스펙이 필요하다고 판단했으나, 재검토 결과 소유권 이전(A/B, 위 trading 항목 참고)으로 해소 가능함이 확인돼 서브프로젝트 3단계(A/B/C)로 진행됐다 — C(신모듈 선언 + 7개 순환 해소)까지 완료돼 strategy-config 이전 전체가 마무리됐다(아래 항목 참고)
