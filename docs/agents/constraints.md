...
- admin 애그리게이트(관리자 조회·정정·재정렬·감사로그·앱오류로그 + 런타임 설정)는 `com.kista.admin`으로 이미 옮겨졌다 — 신규 관련 코드도 레거시 최상위가 아닌 `com.kista.admin` 안에 추가. `domain/model`(flat, admin/settings 서브패키지 없음)이 "domain"으로, `application/usecase`가 "usecase"로, `application/port/output`이 "port"로 NamedInterface 공개 — `application/service`·`adapter/*`는 비공개(internal). event/adapter/schedule NamedInterface 없음(notify 직접 참조 0건이라 알림 이벤트 자체가 없음, `AdminSchedulerController`는 trading/stats "schedule" 소비자지 생산자 아님). `SettingsController`·`ClientErrorLogController`·`GetUserSettingsQuery`·`UserSettingsPort`·`UserSettingsService`·`adapter/out/persistence/settings/`의 `UserSettings*`/`UserNotificationPref*` 6파일·`AdminUserViewAdapter`(레거시 `adapter/out/persistence/user/`)는 user 애그리게이트라 레거시 잔류(향후 user 모듈) — admin이 흡수한 settings는 `RuntimeSettings` API/service/persistence(3파일)뿐. `AdminUserView`/`RuntimeSettings`의 `User`/`Strategy`/`Account` nested enum 참조와 `UserService`/`AccountService`/`StrategyService`의 `RuntimeSettingsPort` 소비는 step 3/4(user/account/strategy-config CLOSED 전환) 정리 대상
