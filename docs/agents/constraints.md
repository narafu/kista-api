...
- privacy 사례(동일 패턴): privacy가 `trading.domain.model.Order`를 빌려 쓰던 것을 `PrivacyOrderType`/`PrivacyOrderDirection`/`FidaPlannedOrder`(`com.kista.privacy.domain.model` 자체 소유, 상수명 byte-identical)로 끊었고 `PrivacyStrategy`(trading)가 `Order.OrderType.valueOf(name())`으로 매핑한다. `PrivacyService`의 notify 직접 호출은 `PrivacyAlertRaisedEvent`(privacy "event" NamedInterface) 발행으로 전환, notify가 `PrivacyAlertNotifier`로 구독 — `privacy→notify→trading→privacy` 전이 순환 해소
- settings 사례(동일 패턴, own-type 4번째 인스턴스): 레거시 `StrategyService.resolveCreationSettings()`/`*CreationResolver`가 `domain/model/settings`(→admin 이전 대상)의 `StrategyCreationSettings`/`StrategyFieldSettings`/`RecurringMode`를 그대로 빌려쓰던 것을 trading `com.kista.trading.domain.strategy` 자체 소유로 끊었고, `StrategyService.resolveCreationSettings()`가 admin→trading 매핑 후 `StrategyCreationResolvers`에 전달(`RecurringMode`는 `valueOf(name())`, 상수명 byte-identical). privacy 사례와 달리 동반 이벤트 전환은 없었다 — `admin→notify` 참조가 0건이라 전이 순환이 성립하지 않음
- admin↔user 사례(own-type 5번째 인스턴스, 포트 역전 변형): user가 admin의 `RuntimeSettingsPort`(가입 승인 설정 전체)를 직접 참조하는 대신, 필요한 조회 1개(`approvalRequiredForUpdate(): boolean`, FOR UPDATE 락)만 담은 `ApprovalPolicyPort`(`com.kista.user.application.port.output`)를 user가 자체 정의하고 admin이 구현한다(broker `MockSimulationDataPort`와 동일한 "포트를 필요로 하는 쪽이 정의, 데이터를 가진 쪽이 구현" 역전 패턴) — Task 2에서 admin↔user 1차 순환 해소 산물
- admin↔account 사례(own-type 6번째 인스턴스, 포트 역전 변형): account가 admin의 `RuntimeSettingsPort`를 직접 참조하는 대신, 필요한 조회 1개(`enabled(Broker): boolean` — sharedkernel.Broker)만 담은 `BrokerEnabledPort`(`com.kista.account.application.port.output`)를 account가 자체 정의하고 admin의 `RuntimeSettingsService`가 구현 — `ApprovalPolicyPort`와 동일 패턴
- broker↔account 사례(own-type 7번째 인스턴스, 값 타입 복제): broker 11개 포트(`BrokerAdapterPort`/`BrokerAccountPort`/`BrokerOrderCorrectionPort`/`SellableQuantityPort`/`ExecutionPort`/`LiveBalancePort`/`PortfolioPort`/`BrokerPricePort`/`MarginPort`/`BrokerConnectionTestPort`, `StockInfoPort`는 원래 Account 무의존)가 `Account`를 그대로 시그니처에 쓰던 것을 broker 자체 소유 `BrokerAccountRef`(id/appKey/secretKey/accountNo/brokerAccountCode + `broker` — `sharedkernel.Broker` 공용 타입, 초기엔 nested 복제였으나 이후 `Account.Broker`와 함께 sharedkernel로 통합 이관돼 복제 삭제)로 끊었다(`com.kista.broker.domain.model`, record 자체는 own-type 존속). `Account→BrokerAccountRef` 변환은 broker가 아닌 `Account.toBrokerRef()` 인스턴스 메서드 1곳이 담당(`com.kista.account.domain.model.Account`) — 이전엔 17개 호출부(trading 13/stats 3/admin 1/strategyconfig 1)가 byte-identical `private static toBrokerRef(Account)` 헬퍼를 각자 복제해뒀던 것을 통합. `Account`가 `broker.domain.model.BrokerAccountRef`를 import하지만 `account→broker` 모듈 엣지는 이미 존재했고(`AccountService`) `broker→account`는 0이라 순환 아님. broker 모듈 어떤 파일도 `Account`를 import하지 않는다. `SellableQuantity`(account 원본과 필드 동일한 복제)와 `BrokerCredentialException`/`BrokerRateLimitException`(account `Account` nested exception 복제, KIS/Toss 인증 실패 시 throw)도 동일 이유로 broker가 own-type 신설 — 이 순환은 계획 작성 시점 사전조사가 "broker→account 참조 0건"으로 잘못 판정했다가 물리 이전 실행 1차 시도의 `verify()`에서 실측 발견됐다(account→broker 엣지가 레거시 OPEN 시절엔 verify() 검사 대상이 아니었던 게 원인)
...
- notify↔trading 사례(own-type 12번째 인스턴스, 값 타입 복제): `RealtimeNotificationPort.notifyTrade`가 `trading.domain.model.TradeEvent`를 시그니처에 직접 쓰던 것을 notify 자체 소유 `TradeEventView`(`com.kista.notify.domain.model`, 필드·Kind enum byte-identical)로 끊었다. 매핑은 이벤트 발생 지점인 `TradingReportNotifier`(notify)가 담당하며, 그 결과 trading 쪽 소비자가 사라진 `trading.domain.model.TradeEvent`는 삭제했다(own-type 신설과 함께 원본이 고아가 된 드문 케이스)
