...
- 매매 코어 애그리게이트(주문/사이클 실행 이력/주문생성 전략)는 `com.kista.trading`으로 이미 옮겨졌다 — 신규 trading 관련 코드도 레거시 최상위가 아닌 `com.kista.trading` 안에 추가. `domain/{model,strategy}`가 "domain"으로, `application/usecase`가 "usecase"로, `application/port/output`이 "port"로, `application/event`가 "event"로, `adapter/in/schedule`이 "schedule"로 NamedInterface 공개 — `application/service`·`adapter/out/*`은 비공개(internal). strategy-config 이전 서브프로젝트 B(2026-09-02~03)로 `StrategyVersion`/`StrategyInfiniteDetail`/`StrategyVrDetail`(+포트·persistence 9파일)과 `VrStrategyLifecycle`(→`VrStrategyDetailUseCase` "usecase" 승격)·`StrategyDetail.VrSummary`(→top-level `VrSummary` "domain" 승격)가 trading으로 합류 — 레거시 `StrategyService`는 이제 이 usecase만 소비. **주문생성 알고리즘 커널은 이후 `com.kista.matching`으로 재이전됨** — `StrategyVrDetail`(값 record)을 포함해 아래 matching 항목 참고, trading엔 영속 버전 부모 `StrategyVersion`/`StrategyInfiniteDetail`·포트·persistence만 남는다
- 주문생성 알고리즘 커널은 `com.kista.matching`으로 이전됐다 — 신규 전략 계산 클래스·position 값객체(InfinitePosition/ReverseModePosition/VrPosition/BootstrapPosition)·주문 어휘 enum(OrderDirection/OrderTiming/OrderType)·`PlannedOrder`·`AccountBalance`·`StrategyVrDetail`은 `com.kista.matching.domain.{model,strategy}`에 추가, `"kernel"` NamedInterface로 병합 공개. 전략 *등록 정책* 리졸버(`StrategyCreationResolver` 계열)는 trading 잔류. 커널은 `com.kista.sharedkernel` + `com.kista.privacy` 외 의존 금지(`HexagonalArchitectureTest.matching_must_not_depend_on_other_modules`)
- 시장 공개 참조 데이터(공포탐욕지수·미국 시장 휴장일 캘린더) 애그리게이트는 `com.kista.market`으로 이미 옮겨졌다 — 신규 관련 코드도 레거시 최상위가 아닌 `com.kista.market` 안에 추가. `domain/model`이 "domain"으로, `application/port/output`이 "port"로, `application/event`가 "event"로 NamedInterface 공개 — `application/{usecase,service}`·`adapter/*`는 비공개(internal — `application/usecase`에 package-info/NamedInterface 없음). `MarketUseCase`(시장 캘린더/캔들 조회)도 `com.kista.market.application.usecase`로 이전 완료 — `MarketHolidayService`가 구현, `MarketHolidayController`가 소비. 레거시 `com.kista.application.usecase`는 비어 소멸했다
- FIDA 기준 매매표(PRIVACY 전략의 전역 SSOT 매매 계획) 애그리게이트는 `com.kista.privacy`로 이미 옮겨졌다 — 신규 관련 코드도 레거시 최상위가 아닌 `com.kista.privacy` 안에 추가. `domain/model`이 "domain"으로, `application/port/output`이 "port"로, `application/usecase`가 "usecase"로, `application/event`가 "event"로 NamedInterface 공개 — `application/service`·`adapter/*`는 비공개(internal). `AdminPrivacyTradeController`/`AdminPrivacyBaseResponse`는 admin 모듈(`com.kista.admin.adapter.in.web`)로 이전됨(`AdminPrivacyBaseResponse`가 privacy의 `PrivacyTradeBaseView`를 "domain" NamedInterface로 소비 — CLOSED admin → CLOSED privacy 소비). PRIVACY *전략 실행* 로직(`PrivacyStrategy`/`PrivacyCycleOrderStrategy` 등 `trading.domain.strategy`)은 trading 소유로 유지 — privacy 모듈은 계획 데이터만 다룬다
- 통계·백테스트·포트폴리오 애그리게이트(사용자 통계·주택/ETF 벤치마크 비교·과거 일봉 백테스트·텔레그램 포트폴리오 조회)는 `com.kista.stats`로 이미 옮겨졌다 — 신규 관련 코드도 레거시 최상위가 아닌 `com.kista.stats` 안에 추가. `domain/model`(+`domain/model/backtest`)이 "domain"으로, `application/usecase`가 "usecase"로, `application/port/output`이 "port"로, `application/event`가 "event"로, `adapter/in/schedule`이 "schedule"로 NamedInterface 공개 — `application/service`·`domain/backtest`·`adapter/in/web`·나머지 `adapter/*`는 비공개(internal). "schedule" 공개는 trading과 동일 이유(`com.kista.web.AdminSchedulerController`가 KbLand 스케쥴러를 수동 트리거용으로 직접 주입). 통계 서비스 3종(`AccountStatisticsService`/`TossStatisticsService`/`BrokerStatisticsRouter`)·`AccountStatisticsUseCase`/`TossStatisticsUseCase`는 CLOSED 전환 당시 account+strategy-config 이전 전(스펙 4단계 대상)이라 레거시 잔류였으나, 두 모듈 이전 완료 후 forward/backward 순환 없음을 재확인해 2026-09-04 `com.kista.stats`로 합류(application.service는 internal, usecase 2개는 "usecase" NamedInterface 편입). `stats→notify` 직접 호출은 `StatsAlertRaisedEvent`(stats "event" NamedInterface) 발행으로 전환, notify가 `StatsAlertNotifier`로 구독 — `stats↔notify` 직접 2-cycle 해소(market/privacy 이벤트 패턴 3번째)
- admin 애그리게이트(관리자 조회·정정·재정렬·감사로그·앱오류로그 + 런타임 설정)는 `com.kista.admin`으로 이미 옮겨졌다 — 신규 관련 코드도 레거시 최상위가 아닌 `com.kista.admin` 안에 추가. `domain/model`(flat, admin/settings 서브패키지 없음)이 "domain"으로, `application/usecase`가 "usecase"로, `application/port/output`이 "port"로 NamedInterface 공개 — `application/service`·`adapter/*`는 비공개(internal). event/adapter/schedule NamedInterface 없음(notify 직접 참조 0건이라 알림 이벤트 자체가 없음, 스케쥴러 수동 트리거 컨트롤러는 `com.kista.web`으로 이전됨). admin이 흡수한 settings는 `RuntimeSettings` API/service/persistence(3파일)뿐 — `SettingsController`·`ClientErrorLogController`·`GetUserSettingsQuery`·`UserSettingsPort`·`UserSettingsService`·`adapter/out/persistence/settings/`의 `UserSettings*`/`UserNotificationPref*` 6파일은 `com.kista.user`로 이미 이전됨(ClientErrorLogController는 admin↔user 2차 순환 해소로 admin에 재배치, 아래 user 애그리게이트 항목 참고). `RuntimeSettings`의 `Strategy`/`Account` nested enum 참조와 `UserService`/`AccountService`/`StrategyService`의 `RuntimeSettingsPort` 소비는 step 4(account/strategy-config CLOSED 전환) 정리 대상 — user 몫은 이미 정리됨(`AdminUserView`/`AdminUserViewPort`가 `com.kista.user`로 이관되며 admin domain/model은 16→15타입, port는 4→3개로 축소)
- user(+auth) 애그리게이트(가입·승인·프로필·설정 + JWT/RefreshToken/블랙리스트/카카오 OAuth)는 `com.kista.user`로 이미 옮겨졌다 — 신규 관련 코드도 레거시 최상위가 아닌 `com.kista.user` 안에 추가. `domain/model`+`domain/auth`가 "domain"으로, `application/usecase`가 "usecase"로, `application/port/output`이 "port"로, `application/event`가 "event"로 NamedInterface 공개 — `application/service`·`adapter/*`·`config`는 비공개(internal). `User.UserRole`/`UserStatus` nested enum + `NotificationType`은 `com.kista.sharedkernel`(`@ApplicationModule(Type.OPEN)`) 독립 타입으로 이관 완료, `NotificationChannel`은 user 단독 소비라 `domain/model` 독립 파일로 잔류(아래 "User nested enum 패턴" 참고). admin에서 이관받은 `AdminUserView`/`AdminUserViewPort`, notify에서 이관받은 `TelegramBotInfoPort`는 물리 이전 후 `verify()` 실측으로 뒤늦게 발견된 admin/notify 순환 해소 산물(스펙 문서 "착수 순서 v2" 3번 항목 각주 참고)
- account 애그리게이트(계좌 자격증명·브로커 연결)는 `com.kista.account`로 이미 옮겨졌다 — 신규 관련 코드도 레거시 최상위가 아닌 `com.kista.account` 안에 추가. `domain/model`이 "domain"으로, `application/usecase`가 "usecase"로, `application/port/output`이 "port"로, `application/event`가 "event"로 NamedInterface 공개 — `application/service`·`adapter/*`는 비공개(internal). `AccountStatisticsService`/`TossStatisticsService`/`BrokerStatisticsRouter`는 실질 stats 관심사라 2026-09-04 `com.kista.stats.application.service`로 이전됨(위 "com.kista.stats/" 참고) — account 신규 코드는 이 3종을 참조하지 않는다. `AccountDeletedEvent`는 account가 발행하고 `AccountCascadeListener`가 구독(이 문단 작성 시점엔 strategy-config가 아직 모듈이 아니라 레거시 위치였으나, 2026-09-07 strategy-config가 trading으로 병합되며 현재는 `com.kista.trading.application.service.AccountCascadeListener`). `BrokerEnabledPort`는 admin↔account 포트 역전 산물(user `ApprovalPolicyPort`와 동일 패턴). broker의 11개 포트가 `Account` 대신 own-type `BrokerAccountRef`를 쓰는 것은 broker↔account 순환 해소 산물 — 물리 이전 실행 1차 시도의 `verify()`에서 실측 발견됐고(계획 작성 시점 사전조사가 놓친 순환), 아래 "모듈 경계 포트 시그니처" own-type 7번째 인스턴스로 해소했다. strategy-config는 이 이전 스코프에서 제외됐었다 — trading과의 결합(`CycleSnapshotCreator.reconfigureVrCycle`)이 실측으로 드러나 별도 스펙이 필요하다고 판단했으나, 재검토 결과 소유권 이전(A/B, 위 trading 항목 참고)으로 해소 가능함이 확인돼 서브프로젝트 3단계(A/B/C)로 진행됐다 — C(신모듈 선언 + 7개 순환 해소)까지 완료돼 strategy-config 이전 전체가 마무리됐다(아래 항목 참고)
- Strategy 애그리게이트(계좌별 영속 전략 설정)는 독립 모듈 `com.kista.strategyconfig`였으나 2026-09-07 `com.kista.trading`으로 병합됐다(→ 아래 "strategyconfig 모듈 병합" 참고) — 신규 관련 코드는 `com.kista.trading.domain.model`(Strategy 등)·`application.usecase`(StrategyUseCase)·`application.port.output`(StrategyPort)·`application.service`(StrategyService, internal)에 추가한다.
- 레거시 `com.kista.adapter`/`com.kista.application` shim은 전부 소멸했다(package-info까지 삭제). 여러 모듈을 집계하는 앱 레벨 inbound 관심사(크로스모듈 컨트롤러·`GlobalExceptionHandler` 등 전역 `@ControllerAdvice`·`@Aspect`·`MetricsConfig`)는 `com.kista.web`(`@ApplicationModule(Type.CLOSED)`, NamedInterface 0개 pure inbound sink)에 추가한다 — 특정 애그리게이트에 속하는 컨트롤러는 그 모듈의 `adapter/in/web`으로 간다. `com.kista.web`은 헥사고날 `..adapter../..application../..domain..` 와일드카드에 안 걸려 `HexagonalArchitectureTest`가 아닌 `ModulithArchitectureTest.verify()`만 policing한다
...
### 모듈 경계 own-type — 정당화 게이트 (2026-09-07 원장 재분류)
...
- `BrokerAccountRef`(broker, `Account`의 자격증명 투영): `Account→BrokerAccountRef` 변환은 `Account.toBrokerRef()` 1곳이 전담. `SellableQuantity`/`BrokerCredentialException`/`BrokerRateLimitException`은 broker 단독 소유 — account 측 원본(`Account.SellableQuantity`/`InvalidBrokerKeyException`/`KisRateLimitException`)은 참조 0건이라 커밋 `036a25ce`로 삭제됨. "복제"가 아니라 broker-native
...
병합으로 own-type 우회 장치 4개가 통째로 소멸했다:
- `trading.domain.model.StrategyRef`: strategyconfig 소유 `Strategy`와 필드 6개가 완전히 동일한 복제였다(값 집합·목적 모두 순환 회피용, 외부 계약 없음) — 병합 후 `Strategy`를 직접 쓰도록 전체 치환
- `StrategyLookupPort`/`StrategyPausePort`(trading이 정의하던 ISP 분리 포트): 병합 후 `StrategyPort`(옛 strategyconfig 소유, 이제 trading 내부)를 직접 쓰면 되므로 소멸. `findTickerById`/`pause` 두 메서드는 `StrategyPort`의 default 메서드로 흡수
- `StrategyLookupAdapter`(위 두 포트의 구현체, `Strategy→StrategyRef` 매핑 어댑터): 매핑 대상 자체가 사라져 소멸

옛 strategyconfig↔admin own-type(`StrategySummary`↔`AdminCycleStrategySummary`)은 병합과 별개로 Task 1에서 `AdminCycleStrategySummary`를 삭제해 정리했다(byte-identical 쌍둥이였다 — 위 "정리된 항목" 참고). `StrategyCreationPolicyPort`(trading 정의·admin 구현 포트 역전)는 병합 후에도 그대로 유지 — admin↔trading 순환 방지 목적이 병합과 무관하게 여전히 유효하다.

**정리된 항목** (더 이상 원장에 없음 — 근본 원인 처리):
- 옛 "settings 사례"(admin `StrategyCreationSettings`/`StrategyFieldSettings`/`RecurringMode` ↔ trading 복제본): 외부 계약에 묶이지 않은 내부 정책 설정이라 (a)(b) 둘 다 미해당 — 2026-09-07 `com.kista.sharedkernel`로 승격, 양쪽 복제본 삭제. `RuntimeSettingsService`의 admin→trading 매핑 코드(`toTradingSettings`/`mapField`/`mapRecurringField`)도 함께 소멸
...
- 옛 "notify↔trading 사례"(`TradeEventView`): 원본 `trading.domain.model.TradeEvent`는 own-type 신설 후 소비자가 사라져 삭제됨(2026-09 이전). 현재 `TradeEventView`는 쌍둥이 없는 notify 단독 소유 — "notify→trading 참조 회피" 근거는 실측상 무효(notify는 이미 `Order`/`Strategy`/`TradingReport`/`CyclePositionHistoryEntry`를 직접 import). 무해하나 게이트 대상 아님
- 옛 "broker/matching PriceSnapshot 사례": `matching.domain.model.PriceSnapshot`이 `"kernel"` NamedInterface에 있었으나 커널 코드는 `BigDecimal` 스칼라만 받아 내부 소비 0 — trading만 소비하는 오배치 고아였다. 2026-09-10 삭제, trading은 `broker.domain.model.PriceSnapshot`을 직접 소비(위 "단일 소유 포트 시그니처 타입" 참고)
...
- ~~`User.UserRole`/`UserStatus`/`NotificationChannel` — 독립 enum 파일 금지, `User` record 내 nested enum~~ **폐기**: user 모듈 CLOSED 전환 시 독립 타입으로 이관. `UserRole`/`UserStatus`는 여러 모듈 공유라 `com.kista.sharedkernel`로(Strategy 4종보다 먼저 — Strategy.Ticker/Type/Status/CycleSeedType도 이후 동일 패키지로 합류, 커밋 a81e76eb, 위 "Account ↔ Strategy 분리" 참고). `NotificationChannel`은 처음엔 sharedkernel로 갔으나 프로덕션 소비가 user 단독이라 2026-09-11 `com.kista.user.domain.model` 독립 파일로 되돌렸다("공유 어휘" 자격 없음 — 이동 후에도 DB `@Enumerated(STRING)` 상수명 byte-identical). DB 컬럼 상수명은 세 enum 모두 byte-identical 유지.
- `Account.Broker`(TOSS/KIS/MOCK)도 `com.kista.sharedkernel.Broker`로 이관 완료 — broker의 `BrokerAccountRef.Broker` byte-identical 복제도 함께 삭제(BrokerAccountRef record만 자격증명 투영 own-type으로 존속). 이로써 여러 CLOSED 모듈이 **같은 값을 공유**하기 위해 nested로 두던 enum은 전부 sharedkernel로 모였다. 전략 생성 정책 어휘(`RecurringMode`/`StrategyCreationSettings`/`StrategyFieldSettings`)도 admin·trading 양쪽 복제본에서 2026-09-07 sharedkernel로 승격됐다(위 "정리된 항목" 참고) — 순환 해소를 위해 의도적으로 **복제 소유**하는(공유하지 않으려고 만든) enum이 새로 필요하면 그때 sharedkernel 통합 대상이 아니지만, 현재 원장에 그런 잔존 항목은 없다. 단 broker `Direction`/`OrderType`·privacy `PrivacyOrderDirection`/`PrivacyOrderType`는 이 원칙의 예외였다가 2026-09-09 own-type 게이트 재판정으로 재분류됨 — wire 포맷이 enum 값 자체엔 실려있지 않아 "외부 계약 분리" 근거가 실제로는 성립하지 않았음이 드러나 `sharedkernel.OrderDirection`/`OrderType`으로 승격, 6개 복제본(matching 자체 소유분 2개 포함) 소멸.
