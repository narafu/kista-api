...
- 기존 예외(관례 이탈, 후속 own-type 대상): `RealtimeNotificationPort`(레거시 shim 해소 때 SSE 레지스트리와 함께 `com.kista.notify.application.port.output`으로 이전, notify "port" NamedInterface)의 `notifyTrade(UUID, TradeEvent)`가 아직 `trading.domain.model.TradeEvent`를 시그니처에 직접 쓴다 — SSE 이동 시 메서드를 동결한 데 따른 것으로, notify 소유 `TradeEventView` + trading 매핑으로 좁히는 것이 own-type 후속 후보
