...
domain/          ← 순수 Java record/class. Spring·JPA 어노테이션 금지
  model/         ← 불변 값 객체 (record 사용), 어그리게이트별 서브패키지
    ...
    strategy/    ← Strategy(+nested: Type{INFINITE/PRIVACY/VR}, Status, Ticker, CycleSeedType), StrategyCycle, CyclePosition, StrategyDetail, CyclePositionHistoryEntry, CycleHistoryPage,
                   ...
                   StrategyVrDetail(strategyVersionId, intervalWeeks, bandWidth, recurringAmount + gradient()/poolLimitRate()),
                   StrategyCycleVrDetail(strategyCycleId, value, gradient, poolLimit),
                   VrPosition(balance, value, bandWidth, poolLimit, poolUsed, firstCycle, cycleDue, remainingTradingDays, recurringAmount
                              + lowerBand/upperBand/buyPrice/sellPrice/nextValue())
  ...
  strategy/      ← InfiniteStrategy/ReverseInfiniteStrategy/PrivacyStrategy/VrStrategy 구현 클래스 (1:1 인터페이스 통폐합됨) — @Component 허용 예외 (ArchUnit)
                   CycleOrderStrategy (인터페이스 + InfiniteCycleOrderStrategy/PrivacyCycleOrderStrategy/VrCycleOrderStrategy 구현): 최소시드·capability SSOT (아래 "CycleOrderStrategy Capability 패턴" 참고)
  ...
  port/out/      ← 아웃바운드 포트 인터페이스
    broker/      ← 브로커 공통 Capability 인터페이스
                   ...
                   BrokerConnectionTestPort: supports()/verifyCredentials(appKey,secretKey,accountId)/verifyAccount(appKey,secretKey,accountNo)→brokerAccountCode (KIS: null, Toss: accountSeq) — 계좌 등록 전 검증이라 Account 없이 broker enum으로 라우팅
...
application/
  service/       ← UseCase 구현체 (package-private @Service), Port를 통해서만 외부 호출, 어그리게이트별 서브패키지
    trading/     ← TradingExecutionFacade(TradingExecutionUseCase 구현 Facade — preview/executeManually/cancelOrder/cancelByCycle/execute/executeBatch 단일 진입점),
                   ...
                   TradingBalanceLoader/TradingOrderPlanner/TradingOrderExecutor/TradingPriceFetcher/TradingReporter/BuyOrderPriceCapper/CycleOrderComputer/BrokerAccountRouter/CyclePositionPersistor/VrCycleRolloverService (helper, package-private)
                   ...
                   CyclePositionPersistor: 포지션 스냅샷 저장 + 사이클 종료·rotation — VR은 endsCycleOnLiquidation()=false로 holdings=0에 사이클 종료 미발동, 포지션 저장 후 VrCycleRolloverService.rollIfDue() 호출
                   VrCycleRolloverService: N주 경과(startDate + intervalWeeks ≤ today) 시 V′ 계산 후 사이클 교체 — V′≤0이면 보류, V′>0이면 기존 사이클 종료 + 새 사이클 원자 생성
    ...
    broker/      ← BrokerAdapterRegistry (public @Component — require(account, Port.class) / find())
                   BrokerConnectionTesters (public @Component — of(broker)로 BrokerConnectionTestPort 라우팅, 계좌 등록 전 자격증명 검증용)
...
adapter/out/
  ...
  toss/          ← Toss API Adapter (TossHttpClient 공통 헤더 처리, TossConfig)
                   TossAuthApi, TossCandleApi, TossHoldingsApi, TossOrderApi, TossPriceApi, TossMarketApi
  ...
  persistence/   ← JPA 인프라 (BaseAuditEntity, BaseCreatedAtEntity, JpaAuditingConfig) + 어그리게이트별 서브패키지
    ...
    strategy/    ← StrategyEntity/StrategyCycleEntity/CyclePositionEntity + 각 JpaRepository + PersistenceAdapter + PersistenceSupport (upsert find-or-create 헬퍼, 패키지 내부 전용)
                   VR 추가: StrategyVrVersionEntity(table=strategy_vr_version) + StrategyVrVersionJpaRepository + StrategyVrDetailPersistenceAdapter (StrategyVrDetailPort 구현)
                            StrategyCycleVrEntity(table=strategy_cycle_vr) + StrategyCycleVrJpaRepository + StrategyCycleVrPersistenceAdapter (StrategyCycleVrPort 구현)
...
- `strategy_vr_version` (`StrategyVrVersionEntity`): 전략 버전별 VR 설정 — intervalWeeks(롤오버 주기), bandWidth(밴드 폭 %), recurringAmount(USD, 양수=입금·0=거치·음수=인출)
  - `gradient()`: recurringAmount<0이면 20(인출 고난도), 그 외 10
  - `poolLimitRate()`: recurringAmount>0→75%, ==0→50%, <0→25%
- `strategy_cycle_vr` (`StrategyCycleVrEntity`): 사이클 시작 시 스냅샷 — value(사이클 기준 V값), gradient, poolLimit
  - 첫 사이클 poolLimit = (`initialValue` + `initialUsdDeposit`) × `poolLimitRate()`
  - 첫 사이클 이후 poolLimit = 롤오버 후 USD pool × `poolLimitRate()` (기존 기준 유지)
- 스케쥴러: `CycleOrderComputer` → `VrCycleOrderStrategy.plan()` → `VrStrategy.buildOrders()` → 매수·매도 사다리 LIMIT+AT_OPEN 주문 생성
  - 첫 사이클 bootstrap: `initialValue`는 기존 TQQQ 평가금, `initialUsdDeposit`은 초기 USD pool
  - 적립식은 둘 다 0이어도 등록 가능하며 due date 당일 `recurringAmount` 예산으로 LOC+AT_CLOSE 매수
  - V만 있으면 poolLimit 금액을 남은 거래일로 나눠 LOC+AT_CLOSE 매도
  - pool만 있으면 poolLimit 금액을 남은 거래일로 나눠 LOC+AT_CLOSE 매수
  - bootstrap LOC 가격: 매수 `currentPrice × 1.10`, 매도 `currentPrice × 0.90`, 수량은 정수 주식 내림
  - 매수 사다리: m=1..20, buyPrice(m) = lowerBand ÷ (holdings + m − 1), poolLimit·pool 한도 내 → 동일 가격 연속 rung 수량 병합
  - 매도 사다리: s=1..20, sellPrice(s) = upperBand ÷ (holdings − s + 1), holdings>20이면 마지막 단에 잔여 전량
  - 가격 캡: buyPrice > currentPrice × 1.10이면 캡 가격으로 교체 (currentPrice=null이면 미적용)
- **holdings=0에도 사이클 유지** — `endsCycleOnLiquidation()=false`, `CyclePositionPersistor`가 종료 미발동
- **N주 롤오버**: `VrCycleRolloverService.rollIfDue()` — 포지션 저장 직후 매일 판정
  - due 조건: `cycle.startDate() + intervalWeeks ≤ today`
  - V′ 계산 → V′≤0이면 롤오버 보류(사이클 유지, 사용자 알림) / V′>0이면 기존 사이클 종료 + 새 사이클 원자 생성
  - 적립식 bootstrap 매수 실패(`recurringAmount>0`, 기존 V=0, holdings=0)는 예외적으로 V=0 새 사이클을 생성해 다음 due date에 다시 LOC 매수를 시도
