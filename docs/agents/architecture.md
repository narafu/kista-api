...
domain/          ← 순수 Java record/class. Spring·JPA 어노테이션 금지
  model/         ← 불변 값 객체(record), 어그리게이트별 서브패키지: user/account/strategy/auth/asset (broker/kis/toss는 com.kista.broker.domain.model로, order 전체 및 strategy 실행 이력 17개는 com.kista.trading.domain.model로, market(공포탐욕지수·시장휴장일)은 com.kista.market.domain.model로, privacy(FIDA 기준 매매표 + PrivacyDates 발행일↔거래일 헬퍼)는 com.kista.privacy.domain.model로, stats(통계 20타입) + backtest(커맨드·결과 5타입)는 com.kista.stats.domain.model(+.backtest)로, admin(관리자 read-model 10 + 런타임 설정 6, 총 16타입)은 com.kista.admin.domain.model(flat — admin/settings 서브패키지 없음)로 이전됨)
...
application/
  usecase/       ← UseCase/Query 인터페이스 (인바운드 포트, 레거시 13개 — 옛 domain/port/in에서 이전됨. 각 모듈의 usecase는 해당 모듈 절 참고)
  port/output/   ← *Port 접미사 아웃바운드 포트 인터페이스 (레거시 12개 — 옛 domain/port/out에서 이전됨. 각 모듈의 port는 해당 모듈 절 참고)
  service/       ← UseCase 구현체 (package-private @Service), Port를 통해서만 외부 호출, 어그리게이트별 서브패키지: account/strategy/user/auth/asset (broker는 com.kista.broker.application.service로, trading은 com.kista.trading.application.service로, market은 com.kista.market.application.service로, privacy는 com.kista.privacy.application.service로, stats/backtest/portfolio는 com.kista.stats.application.service로, admin/settings(RuntimeSettings 계열)는 com.kista.admin.application.service로 이전됨 — 아래 "com.kista.trading/"·"com.kista.market/"·"com.kista.privacy/"·"com.kista.stats/"·"com.kista.admin/" 참고)
...
adapter/in/
  ...
  web/           ← REST Controller + DTO — Auth/Account/TradingCycle/Dashboard/Statistics(KIS 전용 live)/TossStatistics(Toss 전용 live)/Meta(enum SSOT)/Settings(user 셀프서비스)/Fcm/TradeStream(SSE)/Asset/AssetMonthlyCheck/DevAuth(local 전용)/ClientErrorLog(user 클라이언트 오류 수집) — SettingsController·ClientErrorLogController는 user 애그리게이트라 레거시 잔류(향후 user 모듈) (OrderCancelController는 com.kista.trading.adapter.in.web으로, FearGreedController/MarketHolidayController는 com.kista.market.adapter.in.web으로, FidaOrderController(`/api/internal/**`)는 com.kista.privacy.adapter.in.web으로, StatsController(DB 근사 집계)/BacktestController(과거 일봉 시뮬레이션, 계좌 무관)는 com.kista.stats.adapter.in.web으로, Admin* 전체·RuntimeConfigController·AdminSettings/AdminObservability/AdminScheduler/AdminPingController + AdminPrivacyTradeController(privacy 이전 때 admin 홀드오버)는 com.kista.admin.adapter.in.web으로 이전됨 — 전부 internal, NamedInterface 미공개)
...
adapter/out/
  ...
  (persistence/audit/ 폐지됨, persistence/settings/는 RuntimeSettings 3파일만 com.kista.admin.adapter.out.persistence.{audit,settings}로 이전 — UserSettings*/UserNotificationPref* 6파일은 user 애그리게이트라 레거시 persistence/settings/에 잔류, 아래 "com.kista.admin/" 참고)
...
com.kista.admin/    ← Spring Modulith 8번째 이전 모듈(CLOSED) — 관리자 조회·정정·재정렬·감사로그·앱오류로그 + 런타임 설정 애그리게이트, 위 레거시 4패키지와 별개 최상위. "domain"(domain.model)·"usecase"(application.usecase)·"port"(application.port.output) 3개 NamedInterface 공개 — application.service·adapter.*는 의도적으로 비공개(모듈 내부 구현). event NamedInterface 없음(admin→notify 직접 참조 0건, 알림 경로 자체가 없음), schedule/adapter NamedInterface 없음(AdminSchedulerController는 trading/stats "schedule"의 소비자지 생산자가 아님)
  domain/model/       ← flat 16타입 — 관리자 read-model 10(AdminAnomalies/AdminCycleStrategySummary/AdminManualTradeCorrectionCommand/AdminReorderCommand/AdminReorderResult/AdminStats/AdminTradeCorrectionResult/AdminUserView/AppErrorLog/AuditLog) + 런타임 설정 6(RuntimeSettings/StrategyCreationSettings/StrategyFieldSettings/RecurringMode/BenchmarkSettings/BenchmarkFieldSettings) — "domain" NamedInterface로 공개. admin/settings 서브패키지 없음
  application/usecase/ ← AdminQueryUseCase/AdminReorderUseCase/AdminStrategyUseCase/AdminTradeCorrectionUseCase/AdminUserUseCase/AdminSettingsUseCase/RuntimeSettingsUseCase 7개 — "usecase" NamedInterface로 공개(GetUserSettingsQuery는 user 애그리게이트라 레거시 잔류)
  application/port/output/ ← AuditLogPort/AppErrorLogPort/AdminUserViewPort/RuntimeSettingsPort 4개 — "port" NamedInterface로 공개
  ...
  adapter/in/web/     ← internal(비공개) — 컨트롤러 10개(AdminAccount/AdminDashboard/AdminObservability/AdminPing/AdminPrivacyTrade/AdminScheduler/AdminSettings/AdminTrade/AdminUser/RuntimeConfig) + AdminUserViews(응답 조립 헬퍼) + dto/ 20종
  ...
  adapter/out/persistence/settings/ ← internal(비공개) — RuntimeSettingsEntity + RuntimeSettingsJpaRepository + RuntimeSettingsPersistenceAdapter 3파일 (옛 adapter/out/persistence/settings — UserSettings*/UserNotificationPref*는 옮겨오지 않음)
  ── step 3/4 IOU(레거시 forward, 버그 아님, 각 모듈 CLOSED 전환 시 정리): ① `AdminUserView.status`/`role` + `AdminUserViewPort.findAllByStatus(User.UserStatus)`가 레거시 `User.UserStatus`/`UserRole` 직접 참조 — user 모듈 CLOSED 시 own-type 또는 nested enum sharedkernel 이관과 함께 정리. ② `RuntimeSettings`가 `Strategy`/`Strategy.Ticker`/`Strategy.Type`/`Account.Broker` import, `UserService`/`AccountService`/`StrategyService`가 `RuntimeSettingsPort`(admin "port") 소비 — 미래 `user`/`account`/`strategy-config` → admin forward, step 3/4에서 이벤트/own-read 전환 검토. ③ `AdminUserViewAdapter`(레거시 `adapter/out/persistence/user/`, 미래 user)가 `AdminUserViewPort` 구현, `StrategyPersistenceAdapter`(레거시, 미래 strategy-config)가 `AdminCycleStrategySummary` 생산 — 정상 포트 역전(구현자가 admin "port"/"domain" 소비)
