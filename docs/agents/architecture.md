...
application/
  service/       ← UseCase 구현체 (package-private @Service), Port를 통해서만 외부 호출, 어그리게이트별 서브패키지
    trading/     ← TradingExecutionFacade (TradingExecutionUseCase 구현 — preview/executeManually/cancelOrder/cancelByCycle/execute/executeBatch 단일 진입점),
                   ...
                   TradingOrderBudgetAllocator — 계좌별 slot-aware 주문 예산 배정; BUY/SELL 독립, 둘 다 CycleOrderStrategy.allocationPriority() 기준 VR→INFINITE→PRIVACY 우선, BUY는 cap·correction 포함 총액 smaller-first, SELL은 필요 매도수량 smaller-first 후 strategyId/cycleId 오름차순, 승인 방향 병합 시 원본 주문 순서 보존
                   TradingService 계획 경계 — allocator는 계좌별, PLANNED 저장·잔고 부족 알림은 사이클별 실패 격리; 신규 저장 주문과 기존 주문이 모두 없는 사이클은 접수·리포트에서 제외
