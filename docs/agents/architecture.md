...
- 테스트 지원: `com.kista.support`(`DataJpaTestBase`/`WebMvcTestSupport`/`TradingFixtures`)·`application-test.yml`은 `trading-core/src/testFixtures`. com.kista.trading 밖 패키지의 trading-core `@DataJpaTest`는 상위 `@SpringBootConfiguration`이 없으므로 `@ContextConfiguration(classes = TradingCoreJpaTestConfig.class)`를 명시한다. `db/migration`은 루트 전용이라 이를 상대경로로 읽는 trading-core 테스트를 위해 `trading-core`의 `test` 태스크가 `workingDir = rootProject.projectDir`다.
